...
function useFileGetContents($sURL) {
	...
	$contents = @file_get_contents($sURL, false, stream_context_create($options),0, 1200); //12000
	return $contents;
}
...
function useCurl($sURL) {
	...
	$options = array(
		...
		CURLOPT_CONNECTTIMEOUT => 5,	//120  
		CURLOPT_TIMEOUT        => 5,	//120
		CURLOPT_MAXREDIRS      => 2,	//10  
	);
	$ch = curl_init( $sURL );
